...
curl_setopt($ch, CURLOPT_URL, 'https://webapi.drogariasantamarta.com.br/api/login');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
// curl_setopt($ch, CURLOPT_PORT , 31443);
...
// header('User-Agent: campanhaszum/1.0');
...
// $headers[] = 'User-Agent: campanhaszum/1.0';
...
if(!empty($token_recebido)){
    ...
    curl_setopt($ch, CURLOPT_URL, 'https://webapi.drogariasantamarta.com.br/api/custom/aniversario.pontuacao/consultar?cpfcnpj='.$cpfCliente.'&datainicial='.$dataInicio.'&datafinal='.$dataFinal.'&tokenaccess='.$token_recebido);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    if (curl_errno($ch)) {
        ...
        if (!empty($array_pontos)) {
            for ($i=0; $i < count($array_pontos); $i++) { 
                ...
                if(!$db->rows($sel)){
                   ...
                   $id_venda = $db->last_id($insert); 
                    ...
                    $variavel .= '<b>CUPOM JÁ EXISTENTE </b><br>';      
            ...
            $variavel .= '<b>NÃO POSSUI CUPONS A SER GERADO! </b><br>';    
        }
